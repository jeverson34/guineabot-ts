...
export const run: RunFunction = async (client, message, args, prefix) => {
	...
	if (args[0].toLowerCase() === 'get') {
		if (!args[1])
			return await message.channel.send(
				client.embed({ description: 'Please enter the database ID' }, message)
			);
		...
		return await message.channel.send(
			client.embed(
				{
					title: `Successfully deleted ${(args as any[]).splice(1).join(' ')}`,
				},
				message
			)
		);
		...
		const results = await db.set(args[1], (args as any[]).splice(2).join(' '));
		return await message.channel.send(
			client.embed(
				{
					description: `\`\`\`\n${JSON.stringify(results, null, 4)}\n\`\`\``,
