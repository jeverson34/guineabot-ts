...
export const run: RunFunction = async (client, message, args, prefix) => {
	...
	if (!amount)
		return await message.channel.send(
			client.embed(
				{ description: 'Please enter the XP that has value' },
				message
			)
		);
	if (amount < 0)
	...
	await db.set(`${target.id}-${message.guild.id}-level.xp`, amount);
	const newInfo2 = await db.set(
		`${target.id}-${message.guild.id}-level.level`,
		Math.floor(0.1 * Math.sqrt(amount))
	);
...
export const description: string = "Set the XP in someone's XP profile";
